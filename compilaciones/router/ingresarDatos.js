...
router.post("/determinar-error", verificarToken, cargar.single("fileExcel"), async (req, res) => {
    try {
...
router.post("/mandar-excel", verificarToken, cargar.single("fileExcel"), async (req, res) => {
    try {
        //si no hay ningun archivo
        if (!req.file) {
            return res.json({ mensaje: false });
    ...
    try {
        ...
        for (let [clave, valor] of Object.entries(datosIngresados)) {
            ...
            if (nombreColumna) {
                ...
                const valorFinal = (clave === 'serial') ? String(valor).toUpperCase() : valor;
