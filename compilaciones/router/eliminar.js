...
eliminar.post("/desincorporar-datos", verificarToken, async (req, res) => {
    try {
        class Deshabilitar {
            ...
            objetoId;
            datosAcomodados;
            quitarDesincorporacion;
            constructor(objetoId, datosAcomodados, quitarDesincorporacion) {
            ...
            retornarConsulta(columnas, valores) {
                /**
                ...
                return `INSERT INTO ${tabla} (${columnas.join(', ')}) VALUES (${valores.map((_, index) => `$${index + 1}`).join(', ')})`;
            }
            ...
            async ingresarDatosDeshabilitados() {
                for (let filas of this.datosAcomodados) {
                    ...
                    for (let [clave, valor] of Object.entries(filas)) {
                        ...
                        if (!this.columnasDisponibles.includes(clave))
                            continue; //si la clave no esta en las columnas disponibles, se omite
                        //si el valor incluye S/ y la clave no es notas ni ubicacion_actual, se omite, ya que el valor es nulo
                        if (valor_.includes("S/") && clave != 'notas' && clave != 'ubicacion_actual')
                            continue;
                        //se coloca la ubicacion_actual, como la escogida
                        if (clave == 'ubicacion_actual')
                            valor_ = req.body.ubicacion;
