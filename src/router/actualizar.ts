...
actualizar.post("/leer-archivo", verificarToken, cargar.single("actualizar"), async (req:any, res) => {
    try {
        ...
        let columnasNoContadas:any = verificar.comprobarEncabezados();
        ...
        let resultado:any = manejarActualizacion.determinarColumnaId(ids); 
        ...
        if(Object.keys(posibleIngreso).length){
            ...
            const recorrerPosibleIngreso: [string, any][] = Object.entries(posibleIngreso);
...
actualizar.post("/leer-archivo-nuevo", verificarToken, cargar.single("actualizar"), async (req:any, res) => {
