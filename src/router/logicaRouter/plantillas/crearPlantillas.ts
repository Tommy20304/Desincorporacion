...
class CrearPlantilla {
    ...
    async crearPlantilla() {
        ...
        this.datos.forEach((fila, index) => {
            ...
            if (rowNumber >= limiteFinal && limiteFinal) return;
            ...
            Object.entries(fila).forEach(([clave, valor]) => {
                ...
                if (revision) {
                    ...
                    const cell:any = hoja.getCell(`${columnLetter}${rowNumber}`);
        ...
        for(let item = itemMaximo; this.datos.length > item; item++){
            ...
            Object.entries(this.datos[item]).forEach(([clave, valor]) => {
                ...
                if (revision) {
                    ...
                    const cell:any = nuevahoja.getCell(`${columnLetter}${rowNumber}`);
                    cell.value = Array.isArray(valor) ? valor.join("-") : valor;
                }
            });
            rowNumber += 1;
    ...
    async agregarRegistrosFaltantes() {
        ...
        nuevaHoja.columns = columnas.map(item => {
            const headerName:any = Object.values(item)[0];
            return {
        ...
        this.datos.forEach((fila, index) => {
            ...
            if (nroRegistro <= itemMaximo) return;
            ...
            Object.entries(fila).forEach(([clave, valor]) => {
                ...
                if (revision) {
                    ...
                    const columnLetter = this.retornarLetra(colIndex);
                    const cell:any = nuevaHoja.getCell(`${columnLetter}${rowInNewSheet}`);
                    cell.value = Array.isArray(valor) ? valor.join("-") : valor;
                }
            });
        });
    ...
    async crearPlantillaSimple(): Promise<exceljs.Workbook> {
        const libro = new exceljs.Workbook();
...
private aplicarEstilosExcel(hoja: exceljs.Worksheet) {
    /**
