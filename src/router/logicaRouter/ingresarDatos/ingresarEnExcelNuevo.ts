...
class Seleccion {
    ...
    async realizarOperacion(): Promise<void> {
        try {
...
class VerificacionInicial {
    ...
    comprobarEncabezados(): string[] | false {
        ...
        const columnasNoContadas: string[] = [];
        ...
        for (const clave of Object.keys(this.listaErrores["columna repetida"])) {
            ...
            const nombreEncontrado = Object.keys(this.columnasEncontradas).find(
                key => this.columnasEncontradas[key] === clave
            );
            if (nombreEncontrado) delete this.columnasEncontradas[nombreEncontrado];
            }
        }
        }
        ...
        for (let i of listaEncabezados) {
            if (!listaDefinitiva.includes(i)) {
                ...
                if (listaEncabezados.includes(columnaBase)) i = columnaBase;
                }
   ...
   determinarColumna(nombre: string): string | false {
    ...
    const revision = Object.keys(this.sinonimosColumnas).find(key => 
      this.sinonimosColumnas[key].includes(nombreLimpio)
    );
    ...
    if (!revision) {
      ...
      for (const [key, lista] of Object.entries(condicionComponente)) {
        if (lista.some((sinonimo: string) => nombreLimpio.includes(sinonimo))) {
          return key;
        }
      }
      return false;
    ...
    if (this.objeto.length === 0) return;
    
    const claves = Object.keys(this.objeto[0]);

    for (let i = 0; i < claves.length; i++) {
      const datosNulos: string[] = [];
      ...
      if (this.columnasEncontradas[columna]) {
        ...
        if (!this.listaErrores["columna repetida"]) this.listaErrores["columna repetida"] = {};
        
        if (this.listaErrores["columna repetida"][nombreColumnaRepetida]) {
          (this.listaErrores["columna repetida"][nombreColumnaRepetida] as any).push(letraActual);
        } else {
        ...
        if (["categoria", "marca", "modelo", "ubicaciones"].includes(columna)) {
          ...
          if (!registrado) {
            if (["categoria", "marca", "modelo"].includes(columna)) {
              if (!this.posibleIngreso[columna]) this.posibleIngreso[columna] = [];
...
class ComprobarExcepciones extends VerificacionInicial{
    ...
    EtiquetaSerialMacRepetido(date: string, lista: Record<string, string>, columna: string, coordenada: string): boolean {
        ...
        if (Object.keys(lista).includes(date)) {
            ...
            if (datosRepetidos.includes(date)) {
                ...
                (this.listaErrores[nombreError][date] as unknown as string[]).push(coordenada);
            } else {
    ...
    async DeterminarPresencia(): Promise<void> {
        // este metodo convierte el resultado de los parentesis de nuevo en un objeto
        const objetoValido = Object.fromEntries(
            Object.entries(this.columnasEtiquetaSerialMac).filter(([_, valor]) => valor !== "repetido")
        );
        ...
        if (Object.keys(objetoValido).length > 0) {
            ...
            for (const [clave, posicionColumna] of Object.entries(objetoValido)) {
                const valorPosicion = posicionColumna as number;
                const listaDatoTemp: Record<string, string> = {}; // recopila todos los datos, para comprobar que no se repiten
                ...
                for (let filas = 0; filas < this.objeto.length; filas++) {
                    ...
                    if (!datoRaw) continue; // si el dato es nulo, no se hace la consulta
...
class ComprobarComponentes extends ComprobarExcepciones{
    ...
    comprobarColumnasRepetidas(): void {
        ...
        if (erroresRepetidos) {
            ...
            for (const column of Object.values(erroresRepetidos)) {
                const columnLetras = column as string[];
                
                // Se revisa si una columna de componente aparece aquí
                const diagnostico = Object.keys(this.componentes).find((key) => 
                    columnLetras.includes(this.componentes[key][1])
                );
    ...
    determinarComponentes(): void {
        ...
        if (keysComponentes.length <= 1) return;
        ...
        for (let i = 0; i < cantidadFilas; i++) {
            ...
            for (let index = 0; index < keysComponentes.length; index++) {
                ...
                const datosRepetidos = compActual[0][indexCelda].filter((compNombre: string) => 
                    compSiguiente[0][i].includes(compNombre)
                );
...
class ingresarDatos {
    ...
    sinonimos: any;
    verificar: any; // Instancia de la clase de verificación
    fechas: string[];
     ...
     */
    macGuiones(valor:any){
        const macConGuiones: String[] = valor.split('').reduce((acc, letter, index) => {
    ...
    comprobarErrores(dato: string, columna: string): boolean {
        ...
        for (const [clave, lista] of Object.entries(this.verificar.listaErrores)) {
            ...
            for (const [coor, date] of Object.entries(lista as Record<string, any>)) {
                ...
                if (clave === "columna repetida") {
                    ...
                    if ((date as string[]).includes(letraColumna)) return true;
                }
                
                // Caso: Valores repetidos en la misma carga
                if (clave.includes('valor repetido en ')) {
                    if (Object.keys(lista as object).includes(dato)) return true;
                }

                // Caso: Dato ya existente en la base de datos
                if (clave === "dato ya existente" && String(date) === dato) return true;

                // Caso: Errores específicos de la columna
                if (clave.includes(columna) && String(date) === dato) return true;
            }
        }
        return false;
    }
     ...
     */
    async recorrerDatos(): Promise<void> {
        for (const filas of this.datos) {
            ...
            let datosIngresar: Record<string, any> = { 
                fecha_registro: hora, 
            ...
            for (let [columna, dato] of Object.entries(filas)) {
                ...
                if (this.comprobarErrores(datoLimpio, columna)) continue;
                if (this.verificar.retornarDatosSin().includes(datoLimpio.toLowerCase())) continue;
                ...
                if (Object.values(this.verificar.columnasEncontradas).includes(columna)) {
                    ...
                    if (["categoria", "marca", "modelo", "ubicaciones"].includes(claveInterna)) {
                        ...
                        for (const [key, lista] of Object.entries(condicionComponente)) {
                            if ((lista as string[]).some(sin => columna.toLowerCase().includes(sin))) {
                                esComponente = true;
                                break;
                            }
                        }
                        ...
                        if (!esComponente) {
                            if (claveInterna === 'serial') dato = datoLimpio.toUpperCase();
                            if (claveInterna === 'mac_addre') dato = this.macGuiones(dato);
            ...
            for (const [estado, infoComponente] of Object.entries(this.verificar.componentes)) {
                ...
                const filasIndices = (infoComponente as any)[4];
                const componentesNombres = (infoComponente as any)[0];

                for (let i = 0; i < filasIndices.length; i++) {
                    if (filasIndices[i] === (indexFila + 2)) {
                        componentesNombres[i].forEach((nombreOriginal: string) => {
     ...
     */
    async ingresarLosDatos(datos: Record<string, any>): Promise<void> {
        /**
     ...
     */
    async eliminarDatos(): Promise<void> {
