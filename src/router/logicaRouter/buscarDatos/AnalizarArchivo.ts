...
class BuscarIdentificadores {
    ...
    private async buscarPorIdentificador(valor: any, buscarDesincorporacion:any, identificador:any){
        /**
     ...
     */
    private async buscarEquipoEnFila(fila: any, buscarDeshabilitado:any = false) {
        /**
        ...
        for (let [clave, valor] of Object.entries(fila)) {
            if (!valor) continue;
            if(!this.camposDisponibles.includes(clave.trim().toLowerCase())) continue;
